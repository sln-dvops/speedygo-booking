...
export function DeliveryMethod({
  ...
  const parcelDetails = selectedDimensions.map((parcel, index) => {
    ...
    const tierName = determinePricingTier(parcel)
  ...
  const totalBasePrice = parcelDetails.reduce(
    (sum, detail) => sum + detail.basePrice,
    0
  )
      ...
      <Card className="bg-white shadow-lg">
        ...
        <CardContent>
          <p className="text-red-500">
            No parcel weight provided. Please go back and add parcel details.
          </p>
        </CardContent>
